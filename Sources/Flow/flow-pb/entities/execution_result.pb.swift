...
extension Flow_Entities_Chunk: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  public func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    if !self.startState.isEmpty {
      try visitor.visitSingularBytesField(value: self.startState, fieldNumber: 1)
    }
    if !self.eventCollection.isEmpty {
      try visitor.visitSingularBytesField(value: self.eventCollection, fieldNumber: 2)
    }
    if !self.blockID.isEmpty {
      try visitor.visitSingularBytesField(value: self.blockID, fieldNumber: 3)
    }
    if self.totalComputationUsed != 0 {
      try visitor.visitSingularUInt64Field(value: self.totalComputationUsed, fieldNumber: 4)
    }
    if self.numberOfTransactions != 0 {
      try visitor.visitSingularUInt64Field(value: self.numberOfTransactions, fieldNumber: 5)
    }
    if self.index != 0 {
      try visitor.visitSingularUInt64Field(value: self.index, fieldNumber: 6)
    }
    if !self.endState.isEmpty {
      try visitor.visitSingularBytesField(value: self.endState, fieldNumber: 7)
    }
    try unknownFields.traverse(visitor: &visitor)
  }
