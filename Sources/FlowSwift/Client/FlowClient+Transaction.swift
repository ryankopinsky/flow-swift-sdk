...
extension FlowClient {
    public func sendTransaction(_ transaction: FlowTransaction, completion: @escaping(ResultCallback)){
        self.rpcProvider.sendTransaction(transaction: transaction, completion: completion)
    }
    
    public func getTransactionResult(id: FlowIdentifier, completion: @escaping (ResultCallback)) {
        self.rpcProvider.getTransactionResult(id: id, completion: completion)
    }

    public func sendTransaction(script: String, arguments: [CadenceValue], proposer: FlowAddress, payer: FlowAddress, authorizers: [FlowAddress], keychain: FlowKeyChainProtocol, completion: @escaping (ResultCallback)) {
        
        let transaction: FlowTransaction = FlowTransaction()
        
        self.getAccount(account: proposer){
            accountResponse in
